...
export class PrismaCategoryRepository implements ICategoryRepository {
  ...
  async findById(id: string): Promise<Category | null> {
    const prismaCategory = await prismaClient.category.findUnique({
      where: { id },
    });
  ...
  async create(category: Omit<Category, "id" | "createdAt" | "updatedAt">): Promise<Category> {
    const prismaCategory = await prismaClient.category.create({
      data: {
        name: category.name,
  ...
  async update(id: string, data: Partial<Omit<Category, "id" | "createdAt" | "updatedAt">>): Promise<Category> {
    const prismaCategory = await prismaClient.category.update({
      where: { id },
      data: {
        name: data.name,
